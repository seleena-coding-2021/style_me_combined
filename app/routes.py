...
from app.models import User #app.models is needed #from models import User
...
def get_weather_results(zip_code, api_key):
    ...
    r = requests.get(api_url)                             # I am not sure if this works -- at the moment (10/22)
    print(r)   #new code: not apart of the original
    return r.json()

#def get_api_key():
    #config = configparser.ConfigParser()
    #config.read('config.ini')                   #should we remove this block of texts (10/22)
...
@app.route('/name_edit', methods=['GET', 'POST']) #edit your name
@login_required
def name_edit():
...
@app.route('/zipcode_edit', methods=['GET', 'POST'])  #edit your zip code
@login_required
def zipcode_edit():
    if request.method=='GET':
...
@app.route('/birthday_edit', methods=['GET', 'POST'])  #edit your birthday
@login_required
def birthday_edit():
    if request.method=='GET':
